			...
			if (args != null)
			{
				...
				if (param.length > 0)
					if (param[0].equalsIgnoreCase("allowwh") && param.length > 1)
					{
						...
						{
							int state = MySqlDataInsert.simple_get_int("value", "character_variables", "obj_id=" + cm.getObjectId() + " AND name LIKE 'canWhWithdraw'");
							if (state > 0)
							{
								MySqlDataInsert.set("DELETE FROM `character_variables` WHERE obj_id=" + cm.getObjectId() + " AND name LIKE 'canWhWithdraw' LIMIT 1");
								activeChar.sendMessage("Privilege removed successfully");
							}
							else
							{
								MySqlDataInsert.set("INSERT INTO character_variables  (obj_id, name, value, expire_time) VALUES (" + cm.getObjectId() + ",'canWhWithdraw','1',-1)");
								activeChar.sendMessage("Privilege given successfully");
							}
						}
						else
					...
					else if (param[0].equalsIgnoreCase("list"))
					{
						...
						List<Object> list = MySqlDataInsert.get_array(sb.toString());
