	...
	protected void doBypassCommand(Player player, String bypass)
	{
		...
		if ("bbshome".equals(cmd))
		{
			...
			if (dafault.equals(cmd))
			{
				...
				{
					PreparedStatement statement = con.prepareStatement("SELECT count(*) as cnt FROM `bbs_favorites` WHERE `object_id` = ?");
		...
		else if (bypass.startsWith("_bbsbuylist"))
		{
			if (!BBSConfig.GLOBAL_USE_FUNCTIONS_CONFIGS && !checkUseCondition(player))
			{
				onWrongCondition(player);
			...
			// Example: "_bbsbuylist:10000;_bbspage:index" or "_bbsbuylist:10000;_bbshome"
			// or "_bbsbuylist:10000"...
			StringTokenizer st2 = new StringTokenizer(bypass, ";");
			String[] mBypass = st2.nextToken().split(":");
			String pBypass = st2.hasMoreTokens() ? st2.nextToken() : null;
			if (pBypass != null)
			...
			int listId = Integer.parseInt(mBypass[1]);
			BuyListHolder.getInstance().getBuyList(listId, player.getNpcId());
			return;
		}
