...
public class CommunityTeleport extends CustomCommunityHandler
{
	...
	private static final int TELEPORTS_PER_PAGE = 10;
	...
	protected void doBypassCommand(Player player, String bypass)
	{
		...
		if ("cbbsteleport".equals(cmd))
		{
			...
			else if (info instanceof TeleportList)
			{
				...
				html = html.replace("<?teleport_list?>", generateTeleportList(player, (TeleportList) info, page));
	...
	{
		Document doc = null;
		File file = new File(Config.DATAPACK_ROOT, "data/parser/community/bbs_teleports.xml");
		if (!file.exists())
		{
			_log.warn("CommunityTeleport: data/parser/community/bbs_teleports.xml file is missing.");
			return;
		}
	...
	private String generateTeleportList(Player player, TeleportList list, int page)
	{
		int parrentId = 0;
