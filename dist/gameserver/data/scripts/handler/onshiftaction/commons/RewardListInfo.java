...
public abstract class RewardListInfo
{
	...
	public static void showInfo(Player player, NpcInstance npc, RewardType showType, int page)
	{
		...
		else if (npc.getRewardLists().isEmpty())
		...
		else
		{
			...
			for (RewardList list : npc.getRewardLists())
			{
	...
	public static String makeRewardListInfo(Player player, HtmTemplates tpls, NpcInstance npc, List<RewardList> rewardLists, RewardType groupType, String groupName, double penaltyMod, boolean show, int page)
	{
		if (show)
		{
			...
			for (RewardList list : rewardLists)
			{
				...
				{
					if (!g.checkTime(System.currentTimeMillis()))
						continue;

					List<RewardData> items = new ArrayList<RewardData>();
					for (RewardData d : g.getItems())
					{
						if (!d.getItem().isHerb() || groupType == RewardType.EVENT_GROUPED)
							items.add(d);
					}
					...
					if (type == RewardType.RATED_GROUPED)
					{
						if (g.isAdena())
						{
							...
							gcount = rateAdena * npc.getStat().calc(Stats.ADENA_RATE_MULTIPLIER, 1., player, null);
						}
						else
						{
							...
							grate = rateDrop * npc.getStat().calc(Stats.DROP_RATE_MULTIPLIER, 1., player, null);
							gpmod *= npc.getDropChanceMod(player);
							gcount = npc.getDropCountMod(player);
						}
					}
					else if (type == RewardType.SWEEP)
					{
						grate = player.getRateSpoil() * npc.getStat().calc(Stats.SPOIL_RATE_MULTIPLIER, 1., player, null);
						gpmod *= player.getSpoilChanceMod();
						gcount = player.getSpoilCountMod();
					}
					else if (type == RewardType.EVENT_GROUPED)
					{
						...
						gcount = player.getSpoilCountMod() / Config.DROP_COUNT_MODIFIER;
					}
					...
					if (!player.isGM() && (gpmod == 0 || grate == 0 || gcount == 0))
						continue;
					...
					for (RewardData d : g.getItems())
					{
						...
						if (irate > 1.)
							chance = getMinEventsChance(Math.floor(irate), (irate - Math.floor(irate) + 1) * chance, 1);
						else if (irate < 1.)
							chance = getMinEventsChance(1., chance * irate, 1);
					...
					for (RewardData d : items)
					{
						double irate = d.notRate() ? 1.0 : grate;
						double ipmod = d.notRate() ? Math.min(gpmod, 1.) : gpmod;
						double icount = d.notRate() ? 1.0 : gcount;
						...
						long minCount = d.getItem().isStackable() ? Math.round(d.getMinDrop() * icount) : 1L;
						long maxCount = d.getItem().isStackable() ? Math.round(d.getMaxDrop() * icount) : 1L;
						if (irate > 1.)
							maxCount = -maxCount;
