	...
	public void onMenuSelect(Player player, int ask, long reply, int state)
	{
		...
		else if (ask == 401)
		{
			...
			if (player.getClanId() == getOwnerId())
			{
				...
				if (skillEntry.getTemplate().getMpConsume() < getCurrentMp())
				{
					if (!isSkillDisabled(skillEntry.getTemplate()))
					{
		...
		else if (ask == 700)
		{
			if (reply == 1)
			{
				...
				{
					SkillEntry skillEntry = SkillEntry.makeSkillEntry(SkillEntryType.NONE, TELEPORT_SKILL_ID, 1);
					if (skillEntry != null)
						altUseSkill(skillEntry, player);
					else
						_log.warn("Cannot use skill ID[" + skillEntry.getId() + "] LEVEL[" + skillEntry.getLevel() + "]!");
			...
			else if (reply == 3)
			{
				...
				if (skillEntry != null)
					...
					_log.warn("Cannot use skill ID[" + TELEPORT_SKILL_ID + "] LEVEL[1]!");
