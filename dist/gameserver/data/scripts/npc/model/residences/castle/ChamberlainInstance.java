...
public class ChamberlainInstance extends ResidenceManager
{
	...
	public void onBypassFeedback(Player player, String command)
	{
		...
		else if (actualCommand.equalsIgnoreCase("ManageTaxRate"))
		{
			...
			html.setFile("castle/chamberlain/chamberlain-tax.htm");
			html.replace("%tax%", String.valueOf(castle.getTaxPercent()));
			player.sendPacket(html);
		}
		else if (actualCommand.equalsIgnoreCase("SetTaxRate"))
		{
			if (!player.isClanLeader())
			{
				player.sendPacket(SystemMsg.YOU_ARE_NOT_AUTHORIZED_TO_DO_THAT);
	...
	public void onMenuSelect(Player player, int ask, long reply, int state)
	{
		...
		if (ask == -201)
		{
			...
			{
				onBypassFeedback(player, "ManageTaxRate");
			}
			else if (reply == 103)
			{
