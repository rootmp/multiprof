...
public class AuctioneerInstance extends NpcInstance
{
	...
	public void onBypassFeedback(Player player, String command)
	{
		...
		// =============================================================================================
		// Выводит стандартную инфу про КХ(выбор), если єто один из биддер - есть кнопка
		// отменить
		// =============================================================================================
		...
		else if (actualCommand.equalsIgnoreCase("bid_confirm"))
		{
			...
			{
				siegeClan = auctionEvent.newSiegeClan(ClanHallAuctionEvent.ATTACKERS, player.getClanId(), bid, System.currentTimeMillis());
	...
	public void bidOnInstantClanHall(Player player, int instantId)
	{
		...
		{
			// Для внесения ставки за наградной холл необходимо соответствовать условиям
			// участия в аукционе холлов.
			player.sendPacket(SystemMsg.YOU_MUST_HAVE_RIGHTS_TO_A_CLAN_HALL_AUCTION_IN_ORDER_TO_MAKE_A_BID_FOR_PROVISIONAL_CLAN_HALL);
			return;
		}

		SiegeClanObject siegeClan = clanHall.getSiegeEvent().newSiegeClan(SiegeEvent.ATTACKERS, clan.getClanId(), clanHall.getRentalFee(), System.currentTimeMillis());
