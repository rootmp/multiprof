	...
	public void startEvent()
	{
		...
		for (DuelSnapshotObject object : this)
		{
			...
			if (!checkPlayer(player, true))
			{
				...
				// если игрок ищо ТПшится - его невозможно ищо раз сТПшить , переносим на арену,
				// ток без отражения
	...
	protected void announceToPlayersWithValidLevel(String str)
	{
		for (Player player : GameObjectsStorage.getPlayers(false, false))
			if (player.isGM() || player.getLevel() >= _minLevel && player.getLevel() <= _maxLevel)
				player.sendPacket(new SayPacket2(0, ChatType.ANNOUNCEMENT, 0, "", String.format(StringsHolder.getInstance().getString(str, player), _minLevel, _maxLevel)));
	}
