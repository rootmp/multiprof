...
public class CastledataPchLinker
{
	...
	private static final Pattern pattern = Pattern.compile("(?<fort>fortress_begin\\s+\\[(?<f1>\\S+)\\]\\s+(?<f2>\\d+).*?fortress_end)"
			+ "|(?<dominion>dominion_begin\\s+\\[(?<d1>\\S+)\\]\\s+(?<d2>\\d+).*?dominion_end)", Pattern.DOTALL);
	...
	public void load()
	{
		try
		{
			BufferedReader br = Files.newBufferedReader(Config.DATAPACK_ROOT.toPath().resolve(CASTLEDATA_FILE_NAME), Charset.forName("UTF-16"));
