...
public class EffectRestoreHP extends EffectRestoreCP
{
	...
	private double startHeal(Abnormal abnormal, Creature effector, Creature effected)
	{
		...
		else if(!_percent)
		{
			if(!_staticPower)
			{
				double spiritshot_bonus_1 = getSkill().isSSPossible()
						&& getSkill().getHpConsume() == 0 ? (getSkill().isMagic() ? (effector.getChargedSpiritshotPower() / 100.) : 1.) : 1.;
				...
				double base_heal = (power + Math.sqrt(mAtk + mAtk * spiritshot_bonus_1) + staticHealBonus + spiritshot_bonus_3 * magic_level_mod)
						* percentHealBonus;
