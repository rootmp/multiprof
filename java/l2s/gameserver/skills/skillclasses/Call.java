...
public class Call extends Skill
{
	...
	public static IBroadcastPacket canSummonHere(Player activeChar)
	{
		...
		if(activeChar.isInZoneBattle() || activeChar.isInZone(Zone.ZoneType.SIEGE) || activeChar.isInZone(no_restart) || activeChar.isInZone(no_summon)
				|| activeChar.isInBoat() || !activeChar.getReflection().isMain())
	...
	public static IBroadcastPacket canBeSummoned(Creature target)
	{
		...
		if(target.isInZoneBattle() || target.isInZone(Zone.ZoneType.SIEGE) || target.isInZone(no_restart) || target.isInZone(no_summon)
				|| !target.getReflection().isMain() || target.isInBoat())
			return SystemMsg.YOUR_TARGET_IS_IN_AN_AREA_WHICH_BLOCKS_SUMMONING;
