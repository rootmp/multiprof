...
public class AdminSpawnEditor implements IAdminCommandHandler
{
	...
	private String formattedSpawnInfo(Creature obj, int p, Player player)
	{
		...
		final String fileName = SpawnHolder.getInstance().getFileByTemplate(asNpc.getSpawn().getTemplate())
				== null ? "none" : SpawnHolder.getInstance().getFileByTemplate(asNpc.getSpawn().getTemplate());
	...
	private String creatureToTd(int index, Creature creature)
	{
		return "<td width=\"50\">" + index + "</td>" + // 560
				"<td width=\"110\">" + "<a action=\"bypass -h admin_spawneditor_spawninfo 1 " + creature.getObjectId() + "\">" + creature.getName()
				+ "</a></td>" + // 450
				"<td width=\"150\">" + (creature.isNpc() ? ((NpcInstance) creature).getSpawnedLoc().toString() : creature.getLoc().toString()) + "</td>" + // 300
				"<td width=\"260\">"
				+ (creature.isNpc() ? ((NpcInstance) creature).getSpawn().getTemplate()
						!= null ? ((NpcInstance) creature).getSpawn().getTemplate().getName() : "none" : "none")
				+ "</td>" + // 150
				"<td width=\"75\">" + "<button value=\"Remove\" action=\"bypass" + ""/* TODO */
				+ "\" width=73 height=22 back=\"L2UI_CH3.Btn1_normalOn\" fore=\"L2UI_CH3.Btn1_normal\">" + "</td>" + "<td width=\"75\">"
				+ "<button value=\"Info\" action=\"bypass" + ""/*
																												* TODO
																												*/
	...
	{
		String html = "<td width=\"75\" align=left>"
				+ (temp.getName().startsWith("point:") ? "<a action=\"bypass -h admin_spawneditor_point 1 " + temp.getUuid()
						+ "\">point</a>" : temp.getName())
				+ "</td>" + "<td width=\"75\" align=left>"
				+ temp.getNpcList().stream().map(it -> it.getTemplate().getId() + "-" + it.getMax()).collect(Collectors.joining(",")) + "</td>"
				+ "<td width=\"70\" align=left>" + "<a action=\"bypass -h admin_spawneditor_territory 1 " + temp.getUuid() + "\">territory</a></td>"
				+ "<td width=\"15\" align=left>" + "<button action=\"bypass -h admin_spawneditor_markasedited " + page + " " + temp.getUuid() + " " + objectId
				+ "\" width=15 height=15 tooltip=\"click to select template as edited\" back=\"L2UI.CheckBox" + isTemplateEdited(player, temp.getUuid())
				+ "\" fore=\"L2UI.CheckBox" + isTemplateEdited(player, temp.getUuid()) + "\"></button></td>";
		logger.info(html);
		return html;
	}
