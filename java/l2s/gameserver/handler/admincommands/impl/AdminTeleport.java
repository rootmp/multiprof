...
public class AdminTeleport implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(Enum<?> comm, String[] wordList, String fullString, Player activeChar)
	{
		...
		switch(command)
		{
			...
			case admin_change_ref:
				try
				{
					teleportTo(activeChar, activeChar, activeChar.getLoc(), Integer.parseInt(wordList[1]));
				}
				catch(Exception e)
				{
			...
			case admin_teleport:
				...
				int z_loc = ArrayUtils.valid(wordList, 3) != null
						&& !ArrayUtils.valid(wordList, 3).isEmpty() ? Integer.parseInt(wordList[3]) : Config.MAP_MAX_Z;
		...
		switch(command)
		{
			...
			case admin_getref:
				...
				activeChar.sendMessage("Player '" + wordList[1] + "' in reflection: " + activeChar.getReflectionId() + ", name: "
	...
	private void showTeleportCharWindow(Player activeChar)
	{
		...
		replyMSG.append("<button value=\"Teleport near you\" action=\"bypass -h admin_teleport_character " + activeChar.getX() + " " + activeChar.getY()
				+ " " + activeChar.getZ() + "\" width=115 height=15 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_CT1.Button_DF\">");
