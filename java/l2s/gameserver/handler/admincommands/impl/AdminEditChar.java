...
public class AdminEditChar implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(Enum<?> comm, String[] wordList, String fullString, Player activeChar)
	{
			...
			if(fullString.startsWith("admin_settitle"))
				...
				try
				{
					...
					if(target == null || !target.isPlayer())
						target = activeChar;
					/*
					 * TODO @Nexvill Не читает классы выше 136 if(id > (ClassId.VALUES.length - 1))
					 * { activeChar.sendMessage("There are no classes over 136 id."); return false;
					 * }
					 */
				...
				try
				{
					...
					if(target != null && target.isPlayer())
						...
						return false;
					if(MySqlDataInsert.simple_get_int("count(*)", "characters", "`char_name` like '" + val + "'") > 0)
					{
			...
			try
			{
				...
				for(String value : vals)
				{
					if(Strings.isDigit(value))
					{
						...
						{
							sp = NumberUtils.toLong(value, 0L);
							addSp = false;
						}
					}
		...
		else if(fullString.startsWith("admin_setbday"))
		{
			...
			if(!MySqlDataInsert.set("update characters set createtime = UNIX_TIMESTAMP('" + date + "') where obj_Id = "
	...
	private void listCharacters(Player activeChar, int page)
	{
		...
		for(int i = CharactersStart; i < CharactersEnd; i++)
		{
			...
			replyMSG.append("<tr><td width=80>" + "<a action=\"bypass -h admin_character_list " + p.getName() + "\">" + p.getName()
					+ "</a></td><td width=110>" + HtmlUtils.htmlClassName(p.getClassId().getId()) + "</td><td width=40>" + p.getLevel() + "</td></tr>");
		}
	...
	public static void showCharacterList(Player activeChar, Player player)
	{
		...
		replyMSG.append("<tr><td width=100>Class/Id:</td><td>" + HtmlUtils.htmlClassName(player.getClassId().getId()) + "/" + player.getClassId().getId()
				+ "</td></tr>");
		...
		replyMSG.append("<tr><td width=100>Crit/MCrit:</td><td>" + player.getPCriticalHit(null) + "/" + df.format(player.getMCriticalHit(null, null))
				+ "%</td></tr>");
	...
	private void editCharacter(Player activeChar)
	{
		...
		replyMSG.append("<tr><td width=40>HP:</td><td width=70>" + player.getCurrentHp() + "</td><td width=70>" + player.getMaxHp()
				+ "</td><td width=70>Karma: " + player.getKarma() + "</td></tr>");
		replyMSG.append("<tr><td width=40>MP:</td><td width=70>" + player.getCurrentMp() + "</td><td width=70>" + player.getMaxMp()
				+ "</td><td width=70>Pvp Kills: " + player.getPvpKills() + "</td></tr>");
		replyMSG.append("<tr><td width=40>Load:</td><td width=70>" + player.getCurrentLoad() + "</td><td width=70>" + player.getMaxLoad()
				+ "</td><td width=70>Pvp Flag: " + player.getPvpFlag() + "</td></tr>");
		replyMSG.append("</table>");
		replyMSG.append("<table width=270><tr><td>Class<?> Template Id: " + player.getClassId() + "/" + player.getClassId().getId()
				+ "</td></tr></table><br>");
		replyMSG.append("<table width=270>");
	...
	private void showCharacterActions(Player activeChar)
	{
		...
		replyMSG.append("<td width=90><button value=\"Recall\" action=\"bypass -h admin_recall " + player.getName()
				+ "\" width=85 height=20 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_CT1.Button_DF\"></td>");
	...
	private void findCharacter(Player activeChar, String CharacterToFind)
	{
		...
		for(Player element : GameObjectsStorage.getPlayers(true, true))
			if(element.getName().startsWith(CharacterToFind))
			{
				...
				replyMSG.append("<table width=270>");
				replyMSG.append("<tr><td width=80>Name</td><td width=110>Class</td><td width=40>Level</td></tr>");
				replyMSG.append("<tr><td width=80><a action=\"bypass -h admin_character_list " + element.getName() + "\">" + element.getName()
						+ "</a></td><td width=110>" + HtmlUtils.htmlClassName(element.getClassId().getId()) + "</td><td width=40>" + element.getLevel()
						+ "</td></tr>");
				replyMSG.append("</table>");
			}
	...
	private void adminAddExpSp(Player activeChar, long exp, long sp)
	{
