...
public class AdminChangeAccessLevel implements IAdminCommandHandler
{
		...
		switch(command)
		{
			...
			case admin_penalty:
				...
				else
				{
					...
					oId = MySqlDataInsert.simple_get_int("obj_Id", "characters", "`char_name`='" + wordList[1] + "'");
					if(oId > 0)
					{
				...
				if(oId > 0)
					if(Config.BANCHAT_ANNOUNCE_FOR_ALL_WORLD)
						...
						Announcements.shout(activeChar, activeChar + " оштрафовал модератора " + wordList[1] + " на " + count + ", причина: " + reason
	...
	private static void showModersPannel(Player activeChar)
	{
		...
		for(File f : dir.listFiles())
		{
			...
			html += "</td>";
			html += "<td width=45><button width=50 height=20 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_CT1.Button_DF\" action=\"bypass -h admin_moders_log "
					+ oid + "\" value=\"Logs\"></td>";
			html += "<td width=45><button width=20 height=20 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_CT1.Button_DF\" action=\"bypass -h admin_moders_del "
					+ oid + "\" value=\"X\"></td>";
			html += "</tr>";
		}
		html += "</table>";
