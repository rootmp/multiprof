...
public class InstanceZone implements IUserCommandHandler
{
	 ...
	 * @return boolean * @see
	 *         l2s.gameserver.handler.usercommands.IUserCommandHandler#useUserCommand(int,
	 *         Player)
	 */
	@Override
	public boolean useUserCommand(int id, Player activeChar)
	{
		...
		for(int i : activeChar.getInstanceReuses().keySet())
		{
			...
			if(limit > 0)
			{
				...
				activeChar.sendPacket(new SystemMessagePacket(SystemMsg.S1_WILL_BE_AVAILABLE_FOR_REUSE_AFTER_S2_HOURS_S3_MINUTES).addInstanceName(i).addInteger(limit
						/ 60).addInteger(limit % 60));
			}
		}
		if(noLimit)
		{
	 ...
	 * 
	 * @return int[] * @see
	 *         l2s.gameserver.handler.usercommands.IUserCommandHandler#getUserCommandList()
	 */
	@Override
