...
public class Loc implements IUserCommandHandler
{
	...
	public boolean useUserCommand(int id, Player activeChar)
	{
		...
		if(msgId > 0)
			...
			activeChar.sendPacket(new SystemMessage(SystemMessage.S1).addString("Current location : " + activeChar.getX() + ", " + activeChar.getY() + ", "
					+ activeChar.getZ() + ", " + activeChar.getHeading()));

		if(activeChar.isGM())
			activeChar.sendMessage("Map: " + GeoEngine.getMapX(activeChar.getX()) + "_" + GeoEngine.getMapY(activeChar.getY()));
