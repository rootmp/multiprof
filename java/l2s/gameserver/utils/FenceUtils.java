...
{
	public static FenceInstance initFence(String name, int x, int y, int z, int width, int length, int height, FenceState state)
	{
		return new FenceInstance(IdFactory.getInstance().getNextId(), name, width, length, height, state);
	}
	...
	{
		FenceInstance instance = initFence(name, x, y, z, width, length, height, state);
	...
	{
		return spawnFence(name, x, y, z, width, length, height, state, ReflectionManager.MAIN);
	}
}
