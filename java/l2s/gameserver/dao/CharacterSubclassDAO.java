...
public class CharacterSubclassDAO
{
	...
	public static final String SELECT_SQL_QUERY = "SELECT class_id, exp, sp, curHp, curCp, curMp, active, type, active_element_id, sayhas_grace_points FROM character_subclasses WHERE char_obj_id=?";
	public static final String INSERT_SQL_QUERY = "INSERT INTO character_subclasses (char_obj_id, class_id, exp, sp, curHp, curMp, curCp, maxHp, maxMp, maxCp, level, active, type, active_element_id, sayhas_grace_points) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	...
	public boolean insert(int objId, int classId, long exp, long sp, double curHp, double curMp, double curCp, double maxHp, double maxMp, double maxCp, int level, boolean active, SubClassType type, ElementalElement activeElement, int points)
	{
		Connection con = null;
		...
		try
		{
			...
			statement.setInt(14, activeElement.getId());
	...
	public List<SubClass> restore(Player player)
	{
		...
		try
		{
			...
			while(rset.next())
			{
				...
				subClass.setActiveElement(ElementalElement.getElementById(rset.getInt("active_element_id")));
				subClass.setSayhasGrace(rset.getInt("sayhas_grace_points"));
				result.add(subClass);
			}
		}
		catch(Exception e)
	...
	public boolean store(Player player)
	{
		...
		try
		{
			...
			for(SubClass subClass : player.getSubClassList().values())
			{
				...
				sb.append("active_element_id=").append(subClass.getActiveElement().ordinal()).append(",");
				sb.append("sayhas_grace_points=").append(subClass.getSayhasGrace());
