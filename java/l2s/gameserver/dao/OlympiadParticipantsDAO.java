...
public class OlympiadParticipantsDAO
{
	...
	private static final String SELECT_SQL_QUERY = "SELECT char_id, characters.char_name as char_name, character_subclasses.class_id, olympiad_points, olympiad_points_past, olympiad_points_past_static, competitions_done, competitions_loose, competitions_win, game_classes_count, game_noclasses_count, game_team_count FROM olympiad_participants LEFT JOIN characters ON characters.obj_Id = olympiad_participants.char_id LEFT JOIN character_subclasses ON character_subclasses.char_obj_id = olympiad_participants.char_id AND character_subclasses.type=? WHERE characters.obj_Id > 0";
	private static final String REPLACE_SQL_QUERY = "REPLACE INTO `olympiad_participants` (`char_id`, `olympiad_points`, `olympiad_points_past`, `olympiad_points_past_static`, `competitions_done`, `competitions_win`, `competitions_loose`, game_classes_count, game_noclasses_count, game_team_count) VALUES (?,?,?,?,?,?,?,?,?,?)";
	...
	public static final String OLYMPIAD_CLEANUP_PARTICIPANTS = "UPDATE `olympiad_participants` SET `olympiad_points` = ?, `competitions_done` = 0, `competitions_win` = 0, `competitions_loose` = 0, game_classes_count=0, game_noclasses_count=0, game_team_count=0";
	...
	public void select()
	{
		...
		try
		{
			...
			while (rset.next())
			{
				...
				data.setTeamGamesCount(rset.getInt("game_team_count"));
