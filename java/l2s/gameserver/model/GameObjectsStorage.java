...
{
	private static IntObjectMap<GameObject> _objects = new CHashIntObjectMap<GameObject>((int) ((60000 * Config.RATE_MOB_SPAWN)
			+ GameServer.getInstance().getOnlineLimit() + Config.FAKE_PLAYERS_COUNT + 1000));
	...
	public static List<PlayerKiller> getPkPlayers()
	{
		...
		if(now > pk_refresh)
		{
			...
			pk_player = getPlayersStream(player -> player.isPK() && !player.isInPeaceZone()
					&& player.getReflection().getId() == 0 /*&& player.m_SkillMod.p_show_pk_on_map*/).map(PlayerKiller::new).limit(30).toList();
		}
		return pk_player;
	}
}