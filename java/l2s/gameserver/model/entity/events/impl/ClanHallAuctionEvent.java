...
public class ClanHallAuctionEvent extends SiegeEvent<AuctionClanHall, AuctionSiegeClanObject>
{
	...
	public void reCalcNextTime(boolean onStart)
	{
		...
		if(getResidence().getAuctionLength() == 0 && owner == null)
		{
			...
			addOnTimeAction(getResidence().getAuctionLength() * (int) (DAY_IN_MILISECONDS / 1000), new StartStopAction(EVENT, false));
	...
	{
		final Calendar siegeDate = getResidence().getSiegeDate();
		siegeDate.setTimeInMillis(new SchedulingPattern("00 " + siegeDate.get(Calendar.HOUR_OF_DAY) + " * * 1").next(System.currentTimeMillis()));
		siegeDate.add(Calendar.DAY_OF_MONTH, getResidence().getAuctionLength());
	...
	public void stopEvent(boolean force)
	{
		...
		else
		{
			...
			// Если КХ после окончания не обрел хозяина, то выставляем следующую дату
			// аукциона.
			generateSiegeDate();
		}
	...
	public AuctionSiegeClanObject newSiegeClan(String type, int clanId, long param, long date)
	{
		...
		return clan == null ? null : new AuctionSiegeClanObject(this, type, clan, param, date);
	}
