...
public class OlympiadDatabase
{
	...
	public static synchronized void cleanupParticipants()
	{
		...
		{
			con = DatabaseFactory.getInstance().getConnection();
			statement = con.prepareStatement("INSERT INTO olympiad_participants_old SELECT char_id, olympiad_points, olympiad_points_past, olympiad_points_past_static, competitions_done, competitions_win, competitions_loose, game_classes_count, game_noclasses_count, game_team_count FROM olympiad_participants");
			statement.execute();
		...
		for(OlympiadParticipiantData participantsInfo : Olympiad.getParticipantsMap().valueCollection())
		{
			...
			participantsInfo.setTeamGamesCount(0);
		}
	}
