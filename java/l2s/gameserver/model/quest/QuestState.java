...
public final class QuestState
{
	...
	public QuestState(Quest quest, Player player)
	{
		...
		// Save the state of the quest for the player in the player's list of quest
		// onwed
	 ...
	 * <br>
	 * Метод учитывает рейты! 3-ий параметр true/false показывает является ли квест
	 * на профессию и рейты учитываются в завимисомти от параметра
	...
	public void giveItems(int itemId, long count, long limit, boolean rate)
	{
		...
		if(rate)
		{
			if(!Config.RATE_QUEST_REWARD_EXP_SP_ADENA_ONLY || itemId == Quest.ADENA_ID)
	...
	public long rollDrop(long min, long max, double calcChance)
	{
		...
		if(getQuest().getPartyType() != Quest.PARTY_NONE)
		{
			...
			if(player.getParty() != null)
				calcChance *= Config.ALT_PARTY_BONUS[Math.min(Config.ALT_PARTY_BONUS.length, player.getParty().getMemberCountInRange(player, Config.ALT_PARTY_DISTRIBUTION_RANGE))
						- 1];
		}
		if(calcChance > 100)
		{
	...
	/**
	 * Этот метод рассчитывает количество дропнутых вещей в зависимости от рейтов и
	 * дает их, проверяет максимум, а так же проигрывает звук получения вещи. <br>
	 * <br>
	 * Ни один из передаваемых параметров не должен быть равен 0
	 *
	...
	/**
	 * Этот метод рассчитывает количество дропнутых вещей в зависимости от рейтов и
	 * дает их, а так же проигрывает звук получения вещи. <br>
	 * <br>
	 * Следует учесть, что контроль за верхним пределом вещей в квестах, в которых
	 * нужно набить определенное количество предметов не осуществляется. <br>
	 * <br>
	 ...
	 * @param store : Сохраняет в базу и если var это cond обновляет список квестов
	 *              игрока.
	 ...
	 *               Если count передать -1, то будут удалены все указанные
	 *               предметы.
	 ...
	 * <BR>
	 * <U><I>Concept : </I></U> Remove the variable of quest represented by "var"
	 * from the class variable FastMap "vars" and from the database.
	 *
	 * @param var : String designating the variable for the quest to be deleted
	 * @return String pointing out the previous value associated with the variable
	 *         "var"
	 */
	...
	/**
	 * Устанавлевает время, когда квест будет доступен персонажу. Метод используется
	...
	/**
	 * Проверяет, наступило ли время для выполнения квеста. Метод используется для
	 * квестов, которые проходятся один раз в день.
	 * 
	 * @return boolean
	 */
