...
public class Quest implements OnInitScriptListener
{
	...
	public void addQuestItem(int... ids)
	{
		for(int id : ids)
			if(id != 0)
			{
				...
				/*
				 * if(!i.isQuest()) _log.warn("Item " + i +
				 * " multiple is not quest type, but appears in quests - fix ");
				 */
	 ...
	 * <BR>
	 * <U><I>Action : </U></I><BR>
	 * Add state of quests, drops and variables for quests in the HashMap _quest of
	 * L2Player
	 * 
	...
	/**
	 * Add this quest to the list of quests that the passed mob will respond to for
	 * Attack Events.<BR>
	 * <BR>
	 *
	 * @param attackIds
	 ...
	 * {@link l2s.gameserver.network.l2.s2c.ExQuestNpcLogList}
	 * 
	 * @param cond
	 * @param varName
	 * @param killIds
	...
	public void addKillNpcWithLog(int cond, int npcStringId, String varName, int max, int... killIds)
	{
		...
		/*
		 * int npcStringId = 0; if(useNpcString) { npcStringId = getId(); if(npcStringId
		 * >= 10000) npcStringId -= 5000; npcStringId = npcStringId * 100 + 10 + cond; }
		 */
	...
	/**
	 * Add this quest to the list of quests that the passed npc will respond to for
	 * Skill-Use Events.<BR>
	 * <BR>
	 *
	...
	/**
	 * Возвращает название квеста (Берется с npcstring-*.dat) state 1 = "" state 2 =
	 * "In Progress" state 3 = "Done" state 4 = ""
	 */
	...
	public final boolean notifyFirstTalk(NpcInstance npc, Player player)
	{
		...
		// if the quest returns text to display, display it. Otherwise, use the default
		// npc text.
	 ...
	 * <LI><U>"res" ends with string ".html" :</U> an HTML is opened in order to be
	 * shown in a dialog box</LI>
	 * <LI><U>"res" starts with tag "html" :</U> the message hold in "res" is shown
	 * in a dialog box</LI>
	...
	/**
	 * Добавляет спаун с числовым значением разброса - от 50 до randomOffset. Если
	 * randomOffset указан мене 50, то координаты не меняются.
	 */
	...
	/**
	 * Метод для проверки квеста на доступность данному персонажу. Используется при
	 * показе списка квестов, Доступные квесты выделяются синим, недоступные -
	 * красным Также используется для показа соответствующих диалогов
	 *
