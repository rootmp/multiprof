...
public class PlayerBaseStats extends PlayableBaseStats
{
	...
	public double getPAtkSpd()
	{
		...
		if (getOwner().isMounted() && getOwner().getMount().getAtkSpdOnRide() > 0)
			pAtkSpd = getOwner().getStat().calc(Stats.BASE_P_ATK_SPD, getOwner().getMount().getAtkSpdOnRide(), null, null);
		else if (getOwner().isTransformed() && getOwner().getTransform().getBasePAtkSpd() > 0)
			pAtkSpd = getOwner().getStat().calc(Stats.BASE_P_ATK_SPD, getOwner().getTransform().getBasePAtkSpd(), null, null);
		else
