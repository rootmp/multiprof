...
public class MerchantInstance extends NpcInstance
{
	...
	protected void showShopWindow(Player player, int listId, boolean tax)
	{
		...
		if(list == null || list.getNpcId() == getNpcId())
			player.sendPacket(new ExBuySellListPacket.BuyList(list, player, taxRate), new ExBuySellListPacket.SellRefundList(player, false, taxRate));
		else
		{
	...
	{
		return packet == RequestBuyItem.class || packet == RequestSellItem.class || packet == RequestExRefundItem.class
				|| packet == RequestPreviewItem.class || super.canPassPacket(player, packet, arg);
	}
