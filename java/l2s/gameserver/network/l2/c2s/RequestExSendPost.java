	...
	public void run(GameClient client)
	{
		...
		if(_price > 0)
		{
			...
			if((tradeBan != null) && (tradeBan.equals("-1") || (Long.parseLong(tradeBan) >= System.currentTimeMillis())))
			{
				...
				{
					activeChar.sendMessage(new CustomMessage("common.TradeBanned").addString(Util.formatTime((int) ((Long.parseLong(tradeBan) / 1000L)
		...
		else
		{
			...
			if(recieverId > 0)
			{
				...
				if(MySqlDataInsert.simple_get_int("target_Id", "character_blocklist", "obj_Id=" + recieverId + " AND target_Id=" + activeChar.getObjectId())
						> 0) // цель
				// заблокировала
				// отправителя
				{
			...
			if(_count > 0)
			{
				for(int i = 0; i < _count; i++)
				{
					...
					if((item == null) || (item.getCount() < _itemQ[i])
							|| ((item.getItemId() == ItemTemplate.ITEM_ID_ADENA) && (item.getCount() < (_itemQ[i] + serviceCost))) || !item.canBeTraded(activeChar))
					{
