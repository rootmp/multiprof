...
public class SendWareHouseWithDrawList implements IClientIncomingPacket
{
	...
	public void run(GameClient client)
	{
		...
		WarehouseType warehouseType = activeChar.getUsingWarehouseType();
		if(warehouseType != WarehouseType.FREIGHT)
		{
		...
		if(warehouseType == WarehouseType.PRIVATE)
		{
			warehouse = activeChar.getWarehouse();
			logType = Log.WarehouseWithdraw;
		}
		else if(warehouseType == WarehouseType.CLAN)
		{
			...
			if(activeChar.getClan() != null)
				if((activeChar.getClanPrivileges() & Clan.CP_CL_WAREHOUSE_SEARCH) == Clan.CP_CL_WAREHOUSE_SEARCH
						&& (Config.ALT_ALLOW_OTHERS_WITHDRAW_FROM_CLAN_WAREHOUSE || activeChar.isClanLeader() || activeChar.getVarBoolean("canWhWithdraw")))
					canWithdrawCWH = true;
			if(!canWithdrawCWH)
				return;
		...
		else if(warehouseType == WarehouseType.FREIGHT)
		{
		...
		{
			_log.warn("Error retrieving a warehouse object for char " + activeChar.getName() + " - using warehouse type: " + warehouseType);
			return;
		}
