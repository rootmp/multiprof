...
public class SocialActionPacket extends L2GameServerPacket
{
	...
	/*
	 * 22 - Подпрыгивает, вспышка и персонаж исчезает. 31 - Блестки летают вокруг и
	 * кружится персонаж на месте. (Используется автоматически после использования
	 * салона красоты) 32 - Разминается с битой а потом бьет ею. 33 - Танцует как
	 * грузин. 34 - Играет на муз. инструменте. 35 - Дает ляпас. 36 - Кидает
	 * бейсбольный мятч.
	 */
