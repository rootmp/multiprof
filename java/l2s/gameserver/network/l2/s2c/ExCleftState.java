	...
	public boolean write(PacketWriter packetWriter)
	{
		...
		switch (CleftState)
		{
			case CleftState_Total:
				...
				// CatID:%d CatName:%s RemainSec:%d
				// BlueTeam: d[dddd] - Total List TeamID:%d PlayerID:%d Kill:%d Death:%d
				// Tower:%d
				// RedTeam: d[dddd] - Total List TeamID:%d PlayerID:%d Kill:%d Death:%d Tower:%d
				break;
			case CleftState_TowerDestroy:
				// ddddddddd - RemainSec:%d BlueTeamPt:%d RedTeamPt:%d TeamID:%d TowerType:%d
				// PlayerID:%d CleftTowerCount:%d KillCount:%d DeathCount:%d
				break;
			...
			case CleftState_PvPKill:
				...
				// ddddd - PvPKill01 TeamID:%d PlayerID:%d CleftTowerCount:%d Kill:%d Death:%d
				// RemainSec:%d
