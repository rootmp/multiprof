 ...
 * @author nexvill
 */
public class TranscendentInstanceZone50 extends Reflection
{
	// Npcs
	...
	{
		_instanceZone = getZone("[transcendent_instance_zone_50]");
						...
						if (_isFirstEnter)
						{
							...
							_runTask = ThreadPoolManager.getInstance().scheduleAtFixedRate(() ->
							{
								...
								for (int i = 1; i <= 15; i++)
								{
									if (Rnd.get(100) < 70)
									{
										spawners.addAll(spawnByGroup("transcendent_50_variant_" + (variant + 1) + "_" + i));
									}
								}
								for (int i = 1; i <= 12; i++)
								{
									if (Rnd.get(100) < 10)
									{
										spawners.addAll(spawnByGroup("transcendent_50_box_" + i));
									}
								}
								if (System.currentTimeMillis() >= (_timeStart + (240 * 1000)) && !_firstSpawn)
								{
									_firstSpawn = true;
									NpcInstance npc = addSpawnWithoutRespawn(22332, new Location(178184, 20328, -3250, 16578), 0);
			...
			if (!victim.isPlayer())
			{
				if (victim.isNpc())
				{
					if (victim.getNpcId() == 22332)
					{
						...
						if (Rnd.get(100) < 30)
						{
							addSpawnWithoutRespawn(22332, new Location(178184, 20328, -3250, 16578), 0);
						}
					}
				}
				return;
