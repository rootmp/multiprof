 ...
 * @author nexvill
 */
public class TranscendentInstanceZone80 extends Reflection
{
						...
						if(_isFirstEnter)
						{
							...
							_runTask = ThreadPoolManager.getInstance().scheduleAtFixedRate(() -> {
								...
								for(int i = 1; i <= 15; i++)
								{
									if(Rnd.get(100) < 70)
									{
										spawners.addAll(spawnByGroup("transcendent_80_variant_" + (variant + 1) + "_" + i));
									}
								}
								for(int i = 1; i <= 12; i++)
								{
									if(Rnd.get(100) < 10)
									{
										spawners.addAll(spawnByGroup("transcendent_80_box_" + i));
									}
								}
								if(System.currentTimeMillis() >= (_timeStart + (240 * 1000)) && !_firstSpawn)
								{
									_firstSpawn = true;
									NpcInstance npc = addSpawnWithoutRespawn(22335, new Location(129928, 114376, -3785, 16578), 0);
			...
			if(!victim.isPlayer())
			{
				if(victim.isNpc())
				{
					if(victim.getNpcId() == 22335)
					{
						...
						if(Rnd.get(100) < 30)
						{
							addSpawnWithoutRespawn(22335, new Location(129928, 114376, -3785, 16578), 0);
						}
					}
				}
				return;
	...
	{
		_instanceZone = getZone("[transcendent_instance_zone_80]");
