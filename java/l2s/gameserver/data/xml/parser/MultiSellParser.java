...
public class MultiSellParser extends AbstractParser<MultiSellHolder>
{
	...
	{
		return new File(Config.DATAPACK_ROOT, "data/parser/multisell");
	}

	@Override
	public File getCustomXMLPath()
	{
		return new File(Config.DATAPACK_ROOT, "data/parser/multisell/custom");
	}
	...
	protected MultiSellEntry parseEntry(Element n, int multiSellId)
	{
		...
		if (entry.getIngredients().size() == 1 && entry.getProduction().size() == 1 && entry.getIngredients().get(0).getItemId() == 57)
		{
			...
			// fuck it it's spams all the time
			// _log.warn("MultiSell [" + multiSellId + "] Production '" + item.getName() +
			// "' [" + entry.getProduction().get(0).getItemId() + "] price is lower than
			// referenced | " + item.getReferencePrice() + " > " +
			// entry.getIngredients().get(0).getItemCount());
		}
