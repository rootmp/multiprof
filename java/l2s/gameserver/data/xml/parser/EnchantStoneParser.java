...
public class EnchantStoneParser extends AbstractParser<EnchantStoneHolder>
{
	...
	{
		return new File(Config.DATAPACK_ROOT, "data/parser/enchant/enchant_stones.xml");
	}
	...
	protected void readData(Element rootElement) throws Exception
	{
		...
		for(Iterator<Element> iterator = rootElement.elementIterator("enchant_stone"); iterator.hasNext();)
		{
			...
			final String[] grades = enchantStoneElement.attributeValue("grade") == null ? new String[] {
					"NONE"
			} : enchantStoneElement.attributeValue("grade").split(";");
			for(String grade : grades)
				gradesSet.add(ItemGrade.valueOf(grade.toUpperCase()));

			final EnchantType type = enchantStoneElement.attributeValue("type")
					== null ? EnchantType.ALL : EnchantType.valueOf(enchantStoneElement.attributeValue("type"));
			final FailResultType resultType = enchantStoneElement.attributeValue("on_fail")
					== null ? defaultResultType : FailResultType.valueOf(enchantStoneElement.attributeValue("on_fail"));
			final int enchantDropCount = enchantStoneElement.attributeValue("enchant_drop_count")
					== null ? Integer.MAX_VALUE : Integer.parseInt(enchantStoneElement.attributeValue("enchant_drop_count"));
			final int minEnchantLevel = enchantStoneElement.attributeValue("min_enchant_level")
					== null ? defaultMinEnchantLevel : Integer.parseInt(enchantStoneElement.attributeValue("min_enchant_level"));
			final int minFullbodyEnchantLevel = enchantStoneElement.attributeValue("min_fullbody_enchant_level")
					== null ? Math.max(minEnchantLevel, defaultMinFullbodyEnchantLevel) : Integer.parseInt(enchantStoneElement.attributeValue("min_fullbody_enchant_level"));
			final int maxEnchantLevel = enchantStoneElement.attributeValue("max_enchant_level")
					== null ? defaultMaxEnchantLevel : Integer.parseInt(enchantStoneElement.attributeValue("max_enchant_level"));
			final int minEnchantStep = enchantStoneElement.attributeValue("min_enchant_step")
					== null ? 1 : Integer.parseInt(enchantStoneElement.attributeValue("min_enchant_step"));
			final int maxEnchantStep = enchantStoneElement.attributeValue("max_enchant_step")
					== null ? 1 : Integer.parseInt(enchantStoneElement.attributeValue("max_enchant_step"));
