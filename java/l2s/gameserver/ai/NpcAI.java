...
public class NpcAI extends CharacterAI
{
	...
	/*
	 * TODO: Переделать по оффу систему ExPrivates
	 * @Override protected void notifyFriendsOnAttack(Creature attacker, Skill
	 * skill, int damage) { super.notifyFriendsOnAttack(attacker, skill, damage);
	 * if(damage <= 0) return; for(NpcInstance minion : _exPrivates)
	 * minion.getAI().notifyEvent(CtrlEvent.EVT_PARTY_ATTACKED, getActor(),
	 * attacker, damage); }
	 * @Override protected void notifyFriendsOnDie(Creature killer) {
	 * super.notifyFriendsOnDie(killer); for(NpcInstance minion : _exPrivates)
	...
	protected boolean lookNeighbor(int range, boolean force)
	{
		...
		try
		{
			...
			if ((now - _lookNeighborTimestamp) > 500L)
			{
				...
				/*
				 * TODO: Должна ли быть очистка каждые 30 сек? if(force || _lastNeighborsClean +
				 * 30000 < System.currentTimeMillis()) { _lastNeighborsClean =
