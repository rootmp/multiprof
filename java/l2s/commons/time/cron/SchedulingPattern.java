...
public class SchedulingPattern implements NextTime
{
	 ...
	 * @return true if the given string represents a valid scheduling pattern; false
	 *         otherwise.
	 */
	...
	/**
	 * This methods returns next matching timestamp (expressed as a UNIX-era millis
	 * value) according the pattern and given timestamp.
	 * 
	 * @param timezone A time zone.
	 * @param millis   The timestamp, as a UNIX-era millis value.
	...
	public long prev(TimeZone timezone, long millis)
	{
		// TODO: Переписать и оптимизировать.
		GregorianCalendar gc = new GregorianCalendar(timezone);
		gc.setTimeInMillis(millis);
		gc.set(Calendar.SECOND, 0);
		gc.set(Calendar.MILLISECOND, 0);
	...
	public long prev(long millis)
	{
		return prev(TimeZone.getDefault(), millis);
	}
	...
	private interface ValueMatcher
	{
		 ...
		 * 
		 * @param value The value.
		 * @return true if the given value matches the rules of the ValueMatcher, false
		 *         otherwise.
		 */
	...
	private static class IntArrayValueMatcher implements ValueMatcher
	{
		 ...
		 * 
		 * @param integers An ArrayList<Integer> of Integer elements, one for every
		 *                 value accepted by the matcher. The match() method will return
