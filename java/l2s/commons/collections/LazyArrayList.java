...
/**
 * Упрощенная реализация массива с интерфейсом <tt>List</tt> Не
 * потоко-безопасная версия. Порядок элементов при удалении не сохраняется. В
 ...
 * на <tt>capacity * 1.5</tt> При удалении элемента, массив не сокращается,
 * вместо этого последний элемент массива становится на место удаленного.
 * <p>
...
public class LazyArrayList<E> implements List<E>, RandomAccess, Cloneable
{
	...
	/**
	 * Получить список LazyArrayList из пула. В случае, если в пуле нет свободных
	 * объектов, будет создан новый.
	 *
	 ...
	 * @throws IndexOutOfBoundsException в случае, если заданная позиция выходит за
	 *                                   пределы размерности списка
	 */
	@Override
	...
	/**
	 * Вставить указанный элемент в указанную позицию списка, при этом все элементы
	 * в этой позиции сдвигаются направо
	 * 
	...
	/**
	 * Вставить элементы коллекции в указанную позицию списка, при этом все элементы
	 * в этой позиции сдвигаются направо. Для получения элементов коллекции
	 * используется метод {@link Collection#toArray() Collection.toArray()}
	 * 
	...
	/**
	 * Удалить элемент в указаной позиции. При этом элементы не сдвигаются влево, а
	 * в указанную позицию, вместо удаленного, помещается элемент с конца списка
	 * 
	...
	/**
	 * Возвращает true, если объект содержится в списке, в качестве аргумента может
	 * быть <tt>null</tt>, при проверке идиентичности используется оператор
	 * <tt>==</tt>
	 * 
	...
	/**
	 * Возвращает позицию первого вхождения объекта в списке, в качестве аргумента
	 * может быть <tt>null</tt>, при проверке идиентичности используется оператор
	 * <tt>==</tt>, если объект не найден, возвращает -1
	 * 
	 * @param o объект для поиска в списке
	 * @return позиция, в которой находится объект в списке, либо -1, если объект не
	 *         найден
	 */
	@Override
	 ...
	 * аргумента может быть <tt>null</tt>, при проверке идиентичности используется
	 * оператор <tt>==</tt>, если объект не найден, возвращает -1
	 * 
	 * @param o объект для поиска в списке
	 * @return последняя позиция, в которой находится объект в списке, либо -1, если
	 *         объект не найден
	 */
	@Override
	...
	/**
	 * Добавить все элементы коллекции в список. Для получения элементов коллекции
	 * используется метод {@link Collection#toArray() Collection.toArray()}
	 * 
	...
	/**
	 * Удаляет из списка все элементы, которые не содержатся в заданной коллекции,
	 ...
	 * 
	 * @param c коллекция, которая содержит элементы, которые необходимо оставить в
	 *          списке
	 * @return true, если список был изменен
	 */
	@Override
	...
	/**
	 * Удаляет из списка все элементы, которые содержатся в заданной коллекции, для
	 * проверки нахождения элемента в коллекции используется метод коллекции
	 * {@link Collection#contains(Object) Collection.contains(Object)}
	 * 
