...
public class Account
{
	...
	{
		return allowedIpList.isEmpty() || allowedIpList.matches(ip);
	}
	...
	public void update()
	{
		try (Connection con = DatabaseFactory.getInstance().getConnection();)
		{
			PreparedStatement statement = con.prepareStatement("UPDATE accounts SET password = ?, access_level = ?, ban_expire = ?, allow_ip = ?, allow_hwid=?, bonus = ?, bonus_expire = ?, last_server = ?, last_ip = ?, last_access = ?, points = ? WHERE login = ?");
	...
	public static void reducePoints(String account, int count)
	{
		try (Connection con = DatabaseFactory.getInstance().getConnection();)
		{
			PreparedStatement statement = con.prepareStatement("UPDATE accounts SET points = (points - ?) WHERE login = ?");
